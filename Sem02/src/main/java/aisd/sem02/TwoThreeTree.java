...
public class TwoThreeTree {
    ...
    private TreeNode deleteInternal(TreeNode node, int key) {
        if (key == node.key1) {
            ...
            if (predecessor == null) {
                // Если нет левого поддерева, просто удаляем ключ
                if (node.isTwoNode) {
                    return node.right; // Поднимаем правое поддерево
                } else {
                    node.key1 = node.key2;
                    node.isTwoNode = true;
                    return node;
            ...
            if (successor == null) {
                // Если нет правого поддерева, просто удаляем ключ
                if (node.isTwoNode) {
                    return node.left; // Поднимаем левое поддерево
                } else {
                    node.isTwoNode = true;
                    return node;
                }
    ...
    private TreeNode fixThreeNode(TreeNode node) {
        // Аналогичная логика балансировки для 3-узла
        return node;
    }
}